          ...
          {data?.data?.length > 0 ? (
            <div className="flex flex-col items-center gap-3 sm:flex-row">
              {data?.data?.map((item, index) => (
                <div
                  key={item.city_name}
                  className={`w-full cursor-pointer rounded-[5px] py-3 text-center transition-colors duration-300 ${colorClasses[index]} sm:w-[133px]`}
            ...
            <p className="mb-5 text-sm font-normal text-[#0A2A3C]">
              No city available yet!
            </p>
          )}
        </div>
      )}
